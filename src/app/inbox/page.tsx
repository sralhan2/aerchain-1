...
export default function InboxPage() {
  const rows = db.prepare(`SELECT vendor_id, COUNT(*) as n FROM extractions GROUP BY vendor_id`).all() as { vendor_id: string; n: number }[];
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      <header className="border-b border-neutral-200 bg-white px-8 py-5">
        ...
        <p className="text-sm text-neutral-500 mt-1">
          Sent to {VENDOR_META.length} vendors on {RFX.issuedDate} · due {RFX.dueDate} · {RFX.lines.length} line items
        </p>
      </header>
      ...
      <InboxClient initialStatus={initialStatus} />
    </div>
  );
}
