...
export function ComparisonGrid({
  ...
  function cheapestVendorId(lineId: string): string | null {
    ...
    for (const v of vendors) {
      const c = grid[lineId][v.id];
      if (c.status === "quoted" && c.normalizedPriceInr !== null && c.confidence !== null && c.confidence >= 0.7) {
        if (!best || c.normalizedPriceInr < best.price) best = { id: v.id, price: c.normalizedPriceInr };
      }
    }
    return best?.id ?? null;
  }
                            ...
                            <td key={v.id} className="px-4 py-3">
                              <button
                                ...
                                className={`group w-full text-left rounded-md px-2 py-1.5 -mx-2 transition-colors ${
                                  isSelected ? "bg-orange-50 ring-1 ring-orange-300" : "hover:bg-neutral-50"
                                }`}
                              >
