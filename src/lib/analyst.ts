  ...
  input_schema: {
    ...
    properties: {
      answer: {
        type: "string",
        description:
          "A direct, specific answer in plain language — cite vendor names and line items by their description, not internal IDs. If the data doesn't support a confident answer, say so plainly rather than guessing.",
      },
    },
    required: ["answer"],
  },
...
async function buildContext(selectedLineIds?: string[] | null) {
...
export async function answerAnalystQuestion(question: string, history: AnalystTurn[], selectedLineIds?: string[] | null) {
  const context = await buildContext(selectedLineIds);
  ...
  return (toolUse.input as { answer: string }).answer;
}
